<?php
include_once("vascofeed_functions.php");
if (isset($_GET['feed'])) {
	get_vascogames_feed();
}
?>
...
<div class="top-bar">
<h1><?php $translate->__('Install Vascogames.com Feed Games');?></h1>
</div><br />
<div class="select-bar"></div>
<?php
$result = yasDB_select("SELECT COUNT(id) FROM vascogames WHERE hidden = 0"); 
$query_data = $result->fetch_array(MYSQLI_NUM);
$numrows = $query_data[0];
$result->close();
$result2 = yasDB_select("SELECT `sourceid` FROM `gameque` WHERE `source` = 'vasco'");
$queids = array();
...
<script type="text/javascript">
			...
			$("#game").fancybox({
				...
				'height'            : '<?php echo $row['height'];?>'*/
					...
					<form id="kongfeed" action="index.php" method="get">
						<input type="hidden" name="act" value="managevasco">
					  ...
					  <h4><?php $translate->__('Note: Installing a lot of games at once is resource intensive.');?></h4>
                        ...
                        <form enctype="multipart/form-data" action="index.php" method="get">
                        		...
                        		<select name="category" id="category">
									...
									<option value="shooting games">Shooting games</option>
									<option value="sports games">Sports games</option>
									<option value="car games">Car games</option>
	...
	$sql = 'SELECT count(id) FROM vascogames WHERE' . $sql_category . $sql_description . ' installed = 0 && hidden = 0';
...
//**********************************************************************************************************************************************			
	...
	<div class="table">
		<table class="listing" cellpadding="0" cellspacing="0">
			...
			<?php
//*************************************************************************************************************************************************	
		$sql = 'SELECT * FROM vascogames WHERE' . $sql_category . $sql_description . ' installed = 0 AND hidden = 0 ORDER BY id DESC '.$limit;
		$query = yasDB_select($sql,false);
	if($query->num_rows == 0) {
...
<?php
			...
			if (!in_array($gameid, $queids)) {
				echo '<div id="dummy'.$gameid.'"><input type="button" id="qbutton'.$gameid.'" class="button" name="que" value="Queue it" onclick="return doQue('.$gameid.', \'que\', \'vasco\', \''. $row['title'].'\', \''.$thumburl.'\')"/></div><br/><br/>';
			} else {
				echo '<div id="dummy'.$gameid.'"><input type="button" id="qbutton'.$gameid.'" class="button_remove" name="que" value="UN-queue" style="font-size:11px;" onclick="return doQue('.$gameid.', \'remove\', \'vasco\', \''. $row['title'].'\', \''.$thumburl.'\')"/></div><br/>';
			}			
	...
	if ($pageno == 1) {
	   ...
	   echo ' <a href="index.php?act=managevasco&page=1&category='.$category.'&description='.$description.'&filter=Filter games">FIRST</a>';
	   $prevpage = $pageno-1;
	   echo ' <a href="index.php?act=managevasco&page=' . $prevpage . '&category='.$category.'&description='.$description.'&filter=Filter games">PREV</a>';
	} 
