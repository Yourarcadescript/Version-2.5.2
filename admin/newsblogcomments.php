...
<div class="top-bar">
<h1><?php $translate->__('Cpanel - Blog Comments');?></h1>
<div class="breadcrumbs"><a href="index.php?act=comments" title="<?php $translate->__('Comments');?>"><?php $translate->__('Comments');?></a> / <a href="index.php?act=memberscomment" title="<?php $translate->__('Members Comments');?>"><?php $translate->__('Members Comments');?></a></div>
</div><br />
<div class="select-bar">
<label>
<h3><?php $translate->__('Manage News Blog Comments');?></h3>
</label>
</div>
<?php
if(!empty($_GET['edit'])) {
	...
	if($query->num_rows == 0) {
		echo '<center>';
    $translate->__('You cannot edit a members comment that doesnt exist.');
    echo '</center>';
	} else {
		...
		<tr>
	    <th class="full" colspan="2"><?php $translate->__('Edit - Comment');?></th>
	    </tr>
		<tr>
		...
		<tr class="bg">
		<td class="first"><strong><?php $translate->__('Comment');?></strong></td>
	...
	if(!isset($_POST['username']) || !isset($_POST['comment'])) {
		echo '<center>';
    $translate->__('One or more fields was left empty.');
    echo '<br />';
		echo '<a href="index.php?act=newsblogcomments&edit=' . intval($_POST['id']) . '">';
    $translate->__('Click here to go back');
    echo '</a><center>';
	} else {
		...
		echo '<center>';
    $translate->__('Comment Successfully edited!');
    echo '<br />';
		echo '<a href="index.php?act=newsblogcomments">';
	...
	if($query->num_rows == 0) {
		echo '<center>';
    $translate->__('You cannot delete a members comment that does not exist!');
    echo '<br />';
		echo '<a href="index.php?act=newsblogcomments">';
    $translate->__('Click here to go back');
    echo '</a></center>';
		...
		echo '<center>';
    $translate->__('Members comment successfully deleted.');
    echo '<br />';
		echo '<a href="index.php?act=newsblogcomments">';
    $translate->__('Click here to proceed');
    echo '</a></center>';
    ...
    foreach ($_POST['checkbox'] as $box) {
        ...
        if($query->num_rows == 0) {
            echo '<center>';
            $translate->__('You cannot delete this news comment that does not exist!');
            echo '<br />';
            echo '<a href="index.php?act=newsblogcomments">';
            $translate->__('Click here to go back');
            echo '</a></center>';
	...
	if($query->num_rows == 0) {
		echo '<center>';
    $translate->__('No members comments in the database!');
    echo '</center>';
	} else {
		?>
		<div class="table">
		...
		<table class="listing form" cellpadding="0" cellspacing="0">
		<tr>
						<th class="first" width="40px"><?php $translate->__('ID');?></th>
						<th style="width:80px;"><?php $translate->__('News ID');?></th>
						<th><?php $translate->__('Comment');?></th>
						<th><?php $translate->__('Name');?></th>
						<th><?php $translate->__('Date');?></th>
						<th><?php $translate->__('Ip Address');?></th>
						<th width="40px"><?php $translate->__('Edit');?></th>
						<th class="last" ><?php $translate->__('Delete');?></th>
					</tr>
		<?php
		while($row = $query->fetch_array(MYSQLI_ASSOC)) {
			...
			<td><a href="index.php?act=newsblogcomments&edit=<?php echo $row['id'];?>"><font color="#cc0000"><?php $translate->__('Edit');?></font></a></td>
			<td class="last"><a href="index.php?act=newsblogcomments&delete=<?php echo $row['id'];?>" onclick="return confirm(\'Are you sure you want to delete this members comment?\')"><font color="#cc0000"><?php $translate->__('Delete');?></font></a><br/><input name="checkbox[]" type="checkbox" id="checkbox[]" value="<?php echo $row['id'];?>"></td>
			</tr>
			<?php
		}
		$query->close();
