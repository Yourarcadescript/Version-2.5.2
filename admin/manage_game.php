...
<div class="top-bar">
				...
				<h1><?php $translate->__('Cpanel - Games');?></h1>
		  ...
		  <div class="select-bar">
		    <label>
		    <h3><?php $translate->__('Manage Games');?></h3>
		    </label>
		  </div>
<?php
...
if(!empty($_GET['edit'])) {
	...
	if($query->num_rows == 0) {
		...
		<div class="table">
	        ...
	        <tr>
	        <th class="full" colspan="2"><?php $translate->__('Edit - ');?><?php echo $row['title'];?></th>
			</tr>
			<tr class="bg">
			<td class="first"><strong><?php $translate->__('Title');?></strong></td>
			<td class="last"><input type="text" name="title" value="<?php echo $row['title'];?>" style="width:275px;"  /></td>
			</tr>
			<tr>
			<td class="first"><strong><?php $translate->__('Game Height');?></strong></td>
			<td class="last"><input type="text" name="gameheight" value="<?php echo $row['height'];?>" /></td>
		    </tr class="bg">
			<tr class="bg">
			<td class="first"><strong><?php $translate->__('Game Width');?></strong></td>
            ...
            <tr>
			<td class="first"><strong><?php $translate->__('Instructions');?></strong></td><td class="last"></td>
			</tr>
			<tr>
			...
			<tr class="bg">
			<td class="first"><strong><?php $translate->__('Keywords');?></strong></td>
			<td class="last"><textarea name="keywords" style="width:275px;"/><?php echo $row['keywords'];?></textarea></td>
			</tr>
			<tr>
			<td class="first"><strong><?php $translate->__('Category');?></strong></td>
		...
		<?php } ?></select></td>
			...
			<tr class="bg">
			<td class="first"><strong><?php $translate->__('Thumbnail');?></strong></td>
			<td class="last"><input type="text" name="thumbnail" value="<?php echo $row['thumbnail'];?>"  style="width:275px;"/></td>
			</tr>
			<tr>
			<td class="first"><strong><?php $translate->__('File');?></strong></td>
			...
			<tr class="bg">
			<td class="first"><strong><?php $translate->__('Featured');?></strong></td>
			<td class="last">
			...
			<tr>
			<td class="first"><strong><?php $translate->__('Active');?></strong></td>
			<td class="last">
				<select name="active">
			...
			<tr class="bg">
			<td class="first"><strong><?php $translate->__('Game Review');?></strong></td><td class="last"></td>
			</tr>
			<tr>
	...
	if(empty($_POST['title']) || empty($_POST['thumbnail'])) {
		...
		$pid = intval($_POST['id']);
		yasDB_update("UPDATE games SET title = '". yasDB_admin($_POST['title'])."', description = '".yasDB_admin
		($_POST['description'])."', instructions = '".yasDB_admin($_POST['instructions'])."', code = '".trim($_POST['gamecode'])."',keywords = '".yasDB_admin($_POST['keywords'])."', category = ".intval($_POST['category']).", height = ".intval($_POST['gameheight']).", width = ".intval($_POST['gamewidth']).", type = '".yasDB_clean($_POST['type'])."', thumbnail = '".yasDB_clean($_POST['thumbnail'])."', file = '".yasDB_admin($_POST['file'])."', active=".intval($_POST['active']).", review='".yasDB_admin($_POST['review'])."' where id = $pid",false);
    ...
    <form enctype="multipart/form-data" action="index.php" method="get" style="padding:5px;">
		...
		<legend style="color:#EF5F00;font-weight:bold;border:1px solid #9097A9;background-color:#FFFFFF;"><?php $translate->__('Game Filter Search');?></legend>
		<input type="hidden" name="act" value="managegames">
		<?php $translate->__('Title');?>:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" name="title" id="title"/>&nbsp;&nbsp;
		...
		?>
   		...
   		<label for="plays_filter"><?php $translate->__('Game Plays');?></label>
		...
		<?php $translate->__('Featured');?>: <input type="checkbox" name="featured" id="featured">
        </p><br/>
...
<?php
	...
	<form>
	<fieldset style="border:2px solid #9097A9;background-color:#EBEBEB">
	<legend style="color:#EF5F00;font-weight:bold;border:1px solid #9097A9;background-color:#FFFFFF;"><?php $translate->__('Search Results');?></legend>
	<?php $translate->__('Title');?>: &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $box_title; ?><br/>
	<?php $translate->__('Game plays');?>: &nbsp;&nbsp;<?php echo $box_plays; ?><br/>
	<?php $translate->__('Category');?>: &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php if ($box_category != 'all') {echo $catName[$box_category-1];} else { echo $box_category; } ?><br/>
	<?php $translate->__('Description');?>: &nbsp;&nbsp;<?php echo $box_description; ?><br/>
	<?php $translate->__('Keywords');?>: &nbsp;&nbsp;&nbsp;&nbsp;<?php echo $box_keywords; ?><br/>
	<?php $translate->__('Featured');?>: &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $box_featured; ?><br/>
	<?php $translate->__('Games');?>: &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $numrows ?><br/>
	</fieldset>
	</form>
	</div>
	<div style="float:right;margin:40px 40px 0 0;width:305px;">
	<fieldset style="border:2px solid #9097A9;background-color:#EBEBEB">
	<legend style="color:#EF5F00;font-weight:bold;border:1px solid #9097A9;background-color:#FFFFFF;"><?php $translate->__('Exact Search');?></legend>
    ...
    <div class="table">
		...
		<table class="listing" cellpadding="0" cellspacing="0">
			<tr>
			<th class="first" width="86px"><?php $translate->__('Game');?></th>
			<th><?php $translate->__('Plays');?></th>
			<th width="204px"><?php $translate->__('Description');?></th>
			<th><?php $translate->__('Category');?></th>
			<th><?php $translate->__('Featured');?></th>
			<th class="last"></th>
			</tr>
	<?php
	if($query->num_rows==0) {
		...
		while($row = $query->fetch_array(MYSQLI_ASSOC)) {
			...
			<td style="position:relative;height:125px;"><?php echo substr($row['description'],0, 200).'...';?><div style="position:relative; right:0; bottom:0; color:red;"><span class="hintanchor" style="font-weight:bold;cursor:pointer;" onMouseover="showhint('<?php echo addslashes($des);?>', this, event, '300px')"><?php $translate->__('More');?></span></div></td>
