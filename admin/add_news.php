...
<div class="top-bar">
				<h1><?php $translate->__('Cpanel - News');?></h1>
				<div class="breadcrumbs"><a href="index.php?act=newsblogcomments" title="<?php $translate->__('News Blog Comments');?>"><?php $translate->__('News Blog Comments');?></a></div>
			</div><br />
		  <div class="select-bar">
		    <label>
		    <h3><?php $translate->__('Manage News');?></h3>
		    </label>
		  </div>
<?php
if(!empty($_GET['edit'])) {
	...
	if($query->num_rows == 0) {
		...
		<tr>
	        <th class="full" colspan="2"><?php $translate->__('Edit - News');?></th>
			</tr>
			<tr>
			<td class="first" width="172"><strong><?php $translate->__('Date');?></strong></td>
			<td class="last"><input type="text" name="date" value="<?php echo $news['date'];?>"  /></td>
			</tr>
			<tr>
			<td class="first" width="172"><strong><?php $translate->__('News');?></strong></td><td class="last"></td>
			</tr>
			<tr>
			<td style="background-color:#fff;width:100%;"><textarea name="news_text" cols="40" rows="5"><?php echo $news['news_text'];?></textarea></td>
			</tr>
			<tr>
			<td class="first" width="172"><strong><?php $translate->__('Topic');?></strong></td>
	...
	echo '<center>';
  $translate->__('Add News Entry');
  echo '<br/><br/>';?>	
   <form name="add" method="post" action="index.php?act=news">
   <?php $translate->__('Date');?>: <br /><input type="text" name="date" size="35" value="<?php echo date("D, j F Y"); ?>"><br />   
   <?php $translate->__('Topic');?>: <br /><input type="text" name="topic" size="35" /><br />
   <?php $translate->__('News item');?>: <br /><center><textarea name="news_text" rows="4" cols="40"></textarea></center><br />
...
<?php   
	...
	if($query->num_rows == 0) {
		...
		<tr>
						...
						<th style="width:100px;"><?php $translate->__('Topic');?></th>
						<th style="width:100px;"><?php $translate->__('Date');?></th>
						<th ><?php $translate->__('News');?></th>
						<th style="width:40px;"><?php $translate->__('Edit');?></th>
						<th class="last" style="width:40px;"><?php $translate->__('Delete');?></th>
					</tr>
		<?php
		while($news = $query->fetch_array(MYSQLI_ASSOC)) {
			...
			<td><a href="index.php?act=news&edit=<?php echo $news['id'];?>"><font color="#cc0000"><?php $translate->__('Edit');?></font></a></td>
		    <td class="last"><a href="index.php?act=news&delete=<?php echo $news['id'];?>" onclick="return confirm(\'Are you sure you want to delete this news?\')"><font color="#cc0000"><?php $translate->__('Delete');?></font></a><br/><input name="checkbox[]" type="checkbox" id="checkbox[]" value="<?php echo $news['id'];?>"></td>
			</tr>
			<?php
		}
		$query->close();
