...
<div class="top-bar">
<h1><?php $translate->__('Cpanel - Google Analytics');?></h1>
<div class="breadcrumbs"><a href="index.php?act=gainfo" title="<?php $translate->__('Google Analytics Info');?>"><?php $translate->__('Google Analytics Login Info');?></a></div>
</div><br />
<div class="select-bar">
<label>
<h3><?php $translate->__('Google Analytics Data');?></h3>
</label>
</div>
<?php
...
if ($api->login($gaLogin['galogin'], $gaLogin['gapassword'])) {
  ...
  $translate->__('Logging into Google Analytics server failed.');
  echo "</h3><br/>";
  $translate->__('Make sure your login info is correct and try again.');
  echo "</div>";
}
?>
</div>