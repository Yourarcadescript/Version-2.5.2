...
<div class="top-bar">
				...
				<h1><?php $translate->__('Cpanel - Categories');?></h1>
				<div class="breadcrumbs"><a href="index.php?act=general" title="<?php $translate->__('Settings');?>"><?php $translate->__('Settings');?></a> / <a href="index.php?act=links" title="<?php $translate->__('Links');?>"><?php $translate->__('Links');?></a></div>
			</div><br />
		  <div class="select-bar">
		    <label>
		    <h3><?php $translate->__('Manage Categories');?></h3>
		    </label>
		  </div>
<?php
if(!empty($_GET['edit'])) {
	...
	if($query->num_rows == 0) {
		...
		<div class="table">
	        ...
	        <tr>
	        <th class="full" colspan="2"><?php $translate->__('Edit - Category');?></th>
			</tr>
			<tr>
			...
			<td class="last"><?php $translate->__('Active:');?><br/>
		<select name="active">
        ...
        <option value ="no">no</option>
			...
			<tr>
			<td class="first"><strong><?php $translate->__('Display Order');?></strong></td>
			<td class="last"><input type="text" name="order" size=3 value="<?php echo $row['order'];?>"  /></td>
			</tr>
			<tr class="bg">
			<td class="first"><strong><?php $translate->__('Parent category');?></strong></td>
			<td class="last">
			<select name="parent">
            ...
            <tr>
			<td class="first"><strong><?php $translate->__('Show on home page');?></strong></td>
        ...
        <option value ="no">no</option>
			...
			<tr class="bg">
			<td class="first"><strong><?php $translate->__('If not parent,id of the parent category');?></strong></td>
			<td class="last"><input type="text" name="pid" size=3 value="<?php echo $row['pid'];?>"  /></td>
			</tr>
			<tr>
			<td class="first"><strong><?php $translate->__('Meta description');?></strong></td>
	...
	if($query->num_rows == 0) {
		...
		?><center><?php $translate->__('Games are attached to this category. Please choose a category for these games.');?><br />
			<?php
...
if (isset($_GET['page'])) {
	...
	<?php $translate->__('Name:');?> <input type="text" name="name" /> <input type="submit" class="button" value="Add Category" name="add">
	</form></center><br />
	...
	<div class="table">
			...
			<tr>
						<th class="first" width="30px"><?php $translate->__('ID');?></th>
						<th><?php $translate->__('Category');?></th>
						<th width="40px"><?php $translate->__('Active');?></th>
						<th width="30px"><?php $translate->__('Order');?></th>
						<th><?php $translate->__('Parent');?></th>
						<th><?php $translate->__('Sub Cat of (id)');?></th>
						<th><?php $translate->__('Show on home page');?></th>
						<th width="100px"><?php $translate->__('Meta description');?></th>
						<th width="40px"><?php $translate->__('Edit');?></th>
						<th class="last" width="50px"><?php $translate->__('Delete');?></th>
					</tr>
	<?php
	while($row = $query->fetch_array(MYSQLI_ASSOC)) {
		...
		<td style="height:45px;"><a href="index.php?act=categories&edit=<?php echo $row['id'];?>"><font color="#cc0000"><?php $translate->__('Edit');?></font></a></td>
		<td class="last" style="height:45px;">
		<?php
		if ($row['id'] > 10) { ?>
		<a href="index.php?act=categories&delete=<?php echo $row['id'];?>" onclick="return confirm(\'Are you sure you want to delete this category?\')"><font color="#cc0000"><?php $translate->__('Delete');?></font></a><br/><input name="checkbox[]" type="checkbox" id="checkbox[]" value="<?php echo $row['id'];?>">
		<?php } ?>
		</td>
		</tr>
