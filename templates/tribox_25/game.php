...
if (isset($_POST['Favourites_x'])) {
	yasDB_insert("INSERT INTO favourite (userid, gameid) VALUES({$_SESSION['userid']}, {$_POST['gamesave']})") or die("Error updating favourites");
}
...
<!-- AJ-BreadCrumb-End -->
            <div class="game_box">
				...
				<div id="back_button"><A HREF="javascript:history.go(-1)"><img src="<?php echo $setting['siteurl'].'templates/'.$setting['theme'].'/skins/'.$setting['skin'].'/images/back.gif';?>" alt="Go Back" title="Go Back" BORDER="0"/></A></div>
                <div id="banner_ad">
					<?php
				...
				<div id="back_button2"><A HREF="javascript:history.go(+1)"><img src="<?php echo $setting['siteurl'].'templates/'.$setting['theme'].'/skins/'.$setting['skin'].'/images/forward.gif';?>" alt="Go Forward" title="Go Forward" BORDER="0"/></A></div>
				<div id="banner_ad2">
					<?php
				...
				while($games = $query->fetch_array(MYSQLI_ASSOC)) {
					...
					if ($setting['seo']=="yes") {
					    echo '<div class="ran_box">';
						echo'<a href="' . $setting['siteurl'] . 'game/' . $games['id'] . '/' . $gameurl . '.html' . '">';
						echo '<img src="' . $setting['siteurl'] . '' . $thumbnail . '" width="60" height="60" /></a></div>';                         
					}else {
					    echo '<div class="ran_box">';
						echo '<a href="' . $setting['siteurl'] . 'index.php?act=game&id=' . $games['id'] . '">';
						echo '<img src="' . $setting['siteurl'] . '' . $thumbnail . '" width="60" height="60" alt="'.$games['title'].'" title="'.$games['title'].'"/></a></div>';
					}
...
<div class="game_box">
 <div class="game_box_ad"><div class="game_ad">
	...
	<div id="back_button"><A HREF="javascript:history.go(-1)"><img src="<?php echo $setting['siteurl'].'templates/'.$setting['theme'].'/skins/'.$setting['skin'].'/images/back.gif';?>" alt="Go Back" title="Go Back" BORDER="0"</A></div>
	<div id="back_button2"><A HREF="javascript:history.go(+1)"><img src="<?php echo $setting['siteurl'].'templates/'.$setting['theme'].'/skins/'.$setting['skin'].'/images/forward.gif';?>" alt="Go Forward" title="Go Forward" BORDER="0"</A></div></div>
	<div class="game_box_inline">
      <?php
...
<?php
				...
				while($games = $query->fetch_array(MYSQLI_ASSOC)) {
					...
					if ($setting['seo']=="yes") {
					    echo '<div class="ran_box">';
						echo'<a href="' . $setting['siteurl'] . 'game/' . $games['id'] . '/' . $gameurl . '.html' . '">';
						echo '<img src="' . $thumbnail . '" width="60" height="60" /></a></div>';                         
					}else {
					    echo '<div class="ran_box">';
						echo '<a href="' . $setting['siteurl'] . 'index.php?act=game&id=' . $games['id'] . '">';
						echo '<img src="' . $thumbnail . '" width="60" height="60" alt="'.$games['title'].'" title="'.$games['title'].'"/></a></div>';
					}
...
<div class="game_box_inline">
	    ...
	    <?php if ($row['type'] != 'YOUTUBE' && $row['type'] != 'CustomCode') { ?>
            <a href="<?php echo $setting['siteurl'].$row['file'] ;?>" title="Full Screen" target="_blank"><img src="<?php echo $setting['siteurl'];?>templates/<?php echo $setting['theme'];?>/skins/<?php echo $setting['skin'];?>/images/buttons/fullscreen.png" width="100" height="30" border="0" alt="full screen" /></a>&nbsp;&nbsp;
        <?php } ?>
...
<div class="game_header">info:</div>   
...
<div id="ratingbox">
    ...
    <div id="gamecode">
		...
		$query->close();
		?>
        <textarea style="width: 290px; height: 100px; border:solid 3px #999; font: normal 12px arial;" id="txtarea" onClick="SelectAll('txtarea');">
        <?php 
        if($row['type']=='SWF') {
			echo '<div id="gameDiv" style="height: '.$new_height.'px; width: '.$new_width.'px; position:relative; top: 0; left: 0">';
			include "media/swf.php";
...
<div class="game_header">comments:</div>   

<div class="game_box" id="messages">
	...
	if($total == 0) {
		...
		while($row = $query->fetch_array(MYSQLI_ASSOC)) {
			...
			echo '<div class="comment_box1">' . $row['name'] . '</div>
			<div class="comment_box2">' . $text . '</div>';
		}
	}
	$query->close();
...
<div class="game_header">Leave a comment:</div>   
<div class="game_box">
	<div class="comment_box"><div id="preview" style="text-align:center;"></div>
	<div id="commentBox">
	<?php if(isset($_SESSION['user'])) {
	?>
	<div id="msg"><center>
		<form name="comment" id="addcomment" method="post" action="<?php echo $setting['siteurl'];?>includes/comment.php">
			...
			<textarea name="comment" rows="3" cols="26" id="comment_message"><?php echo $comment; ?></textarea><br />
	...
	<div class="comment_box4"><center>
        ...
        <input name="sendcomment" type="submit" value="Add Comment" style="border: 1px solid #000; margin-top: 2px;" />
		</form></center></div>
		<?php
	} else {
		?><span style="display:block;text-align:center;font-size:24px;padding:10px 0 50px 0;">Please Sign in or Register to leave a comment</span><?php
	}
	?>
	</div>
	</div>
