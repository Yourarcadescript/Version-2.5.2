...
<div class="forumcontainer"><div class="forumheader">Statistics</div>
...
<div class="middlewrapperbox">
<?php
$query = yasDB_select("select * from forumtopics order by views desc limit 5");
while($row = $query->fetch_array(MYSQLI_ASSOC)) {
	$forumurl = $row['subject'];
	$forumurl = str_replace(" ", "-", $forumurl);
	$forumurl = str_replace("'", "_", $forumurl);
	$forumurl = str_replace('"', "_", $forumurl);
	$forumurl = str_replace('/', "_", $forumurl);
	$forumurl = str_replace("\\", "_", $forumurl);
	$forumurl = rawurlencode($forumurl);
	if ($setting['seo']=='yes') {
		echo'<a href="' . $setting['siteurl'] . 'forumtopics/' . $row['id'] . '/' . $forumurl . '.html' . '">' . $row['subject'] . '</a> (Views:  ' . $row['views'] . ')<br>';
	} else {
		echo'<a href="' . $setting['siteurl'] . 'index.php?act=forumtopics&id=' . $row['id'] . '">' . $row['subject'] . '</a> (Views:  ' . $row['views'] . ')<br>';
	}
}
$query->close();
?>
...
while($row = $query->fetch_array(MYSQLI_ASSOC)) {
	...
	$forumurl = rawurlencode($forumurl);
	if ($setting['seo']=='yes') {
		echo'<a href="' . $setting['siteurl'] . 'forumtopics/' . $row['id'] . '/' . $forumurl . '.html' . '">' . $row['subject'] . '</a>  posted on ' . $row['date'] . '<br>';
	} else {
		echo'<a href="' . $setting['siteurl'] . 'index.php?act=forumtopics&id=' . $row['id'] . '">' . $row['subject'] . '</a> posted on ' . $row['date'] . '<br>';
	}
}
$query->close();
?>
...
<tr>
<td width="25%" align="center">New Members</td>
<td width="25%" align="center">Top Posters</td>
<td width="25%" align="center">Top Authors</td>
<td width="25%" align="center">Most Replies</td>
</tr>
<tr>
